...
Checks AI crawler access, llms.txt, schema, citability, and content structure.
Runs on GitHub Actions with no paid APIs.
"""
import os
import re
import json
...
def check_robots():
    ...
    for crawler in AI_CRAWLERS:
        # Look for Disallow rules targeting each crawler
...
def check_llms_txt():
    ...
    try:
        ...
        if status == 200:
            ...
            return {"found": True, "lines": len(lines), "title": has_title, "desc": has_description}
    except Exception as e:
        pass
    print("  ❌ Missing")
    return {"found": False}
...
def check_citability(soup):
    ...
    # Check for lists (AI models love lists)
    lists = main.find_all(["ul", "ol"])
    if len(lists) < 2:
        notes.append(f"Only {len(lists)} lists (aim for 3+)")
        score -= 5

    # Check for tables (data tables are citation magnets)
    tables = main.find_all("table")
    if len(tables) >= 1:
        notes.append(f"{len(tables)} data table(s) found")

    # Check for headings as questions
    headings = main.find_all(["h2", "h3"])
    question_headings = [h for h in headings if "?" in h.get_text() or h.get_text().lower().startswith(("how", "what", "why", "when", "where"))]
...
def main():
    ...
    for url in urls[:20]:  # Limit to 20 for speed
    ...
    if robots.get("blocked"):
        print(f"\n⚠️  {len(robots['blocked'])} AI crawlers blocked — fix before AI search traffic grows")
        return 1
